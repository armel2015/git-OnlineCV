...
var scripts = document.getElementsByTagName('script'),
	...
	//definition de la durée du cookie
...
var tarteaucitron = {
          ...
          if(tarteaucitronCustomText !== ''){
            ...
            tarteaucitron.addScript(pathToServices, '', function () {
                ...
                var body = document.body,
                    ...
                    index,
                    orientation = 'Top',
                    cat = ['necessaires', 'marketing', 'statistiques', 'preferences'],
                    i;
                ...
                if (tarteaucitron.parameters.highPrivacy && !tarteaucitron.parameters.AcceptAllCta) {
                    ...
                    html += '      <ul class="tarteaucitronBorder" id="catOfCookies">';
                        for (i = 0; i < cat.length; i += 1) {
                            html += '      	 <li>';
					...
					html += '   <button type="button" id="tarteaucitronPersonalize" onclick="tarteaucitron.userInterface.respondAllByAllCat();">';
                    html += 					   tarteaucitron.lang.valider;
                    html += '   </button>';
                    html += '     </ul>';
    ...
    "addService": function (serviceId) {
        ...
        if (tarteaucitron.added[service.key] !== true) {
            ...
            html += '           &#10003 ' + tarteaucitron.lang.allow;
            html += '       </button> ';
    ...
    "userInterface": {
		...
		//gestion du parametrage des cookies par catégories
		
		//"use strict" = permet d'utiliser d'exécuter la fonction uniquement avec les variables déclarées
		//tarteaucitron.services = liste des services pris en compte par tac dans tarteaucitron.service.js
		//tarteaucitron.job = job ajouté permettant la prise en compte du service par TAC exp:(tarteaucitron.job = tarteaucitron.job || []).push('analytics');
		"respondAllByCat": function (cathegory) {
            "use strict"; //permet d'utiliser d'exécuter la fonction uniquement avec les variables déclarées
			var type = cathegory.id,
				status = cathegory.checked;
            ...
            for (index = 0; index < tarteaucitron.job.length; index += 1) {
				...
				//met à jour le status uniquement pour les services qui sont de même type que le parametre
		...
		"respondAllByAllCat": function () {
            "use strict"; //permet d'utiliser d'exécuter la fonction uniquement avec les variables déclarées
			var catNess = document.getElementById("necessaires"),
				catMarkets = document.getElementById("marketing"),
				catPrefs = document.getElementById("preferences"),
				catStats = document.getElementById("statistiques"),
				
				categories = [catNess, catMarkets, catPrefs, catStats],
            ...
            for (index = 0; index < tarteaucitron.job.length; index += 1) {
                service = s[tarteaucitron.job[index]];
				key = service.key;
				if(service.type === "necessaires"){
					status = catNess.checked;
				}else if(service.type === "marketing"){
					status = catMarkets.checked;
				}else if(service.type === "preferences"){
					status = catPrefs.checked;
				}else if(service.type === "statistiques"){
					status = catStats.checked;
				}
        ...
        "respond": function (el, status) {
            ...
            if (status === false && tarteaucitron.launch[key] === true) {
				//Si on deny le cookie et que le service est chargé ou enregistré alors on recharge la page
				//Si la page est rechargé => appel de la function window.location.reload();
                tarteaucitron.reloadThePage = true;
            }

            // if not already launched... launch the service
			// Si on allow le cookie, si il n'est pas encore chargé on le charge premièrement
			// Ensuite on fait appel au script qui va charger les datalayers et faire appel au lien de googletagmanager.com avec l'ID de tarteaucitron tagmanager
            if (status === true) {
			...
			//le cookie est ensuite crée puis on change la couleur du bouton
            tarteaucitron.state[key] = status;
            tarteaucitron.cookie.create(key, status);
            tarteaucitron.userInterface.color(key, status);
        },
